...
// GET all drivers (supporting status filter, license expiry warning flags, and search)
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    // Map profiles and append boolean virtual column for expiration state
...
// POST create driver profile (Managers only)
...
// PUT update driver profile (Managers & Safety Officers)
router.put('/:id', authenticateToken, verifyRole(['Fleet Manager', 'Safety Officer']), async (req, res) => {
  try {
    const driver = await Driver.findByPk(req.params.id);
    if (!driver) {
      return res.status(404).json({ message: 'Driver not found' });
    ...
    // Edge Case: Block manual status updating if driver is active on a trip
...
// DELETE driver profile (Managers only)
router.delete('/:id', authenticateToken, verifyRole(['Fleet Manager']), async (req, res) => {
  try {
    ...
    // Prevent deletion if currently active on a trip
